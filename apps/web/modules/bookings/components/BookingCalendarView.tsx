  ...
  const events = useMemo<CalendarEvent[]>(() => {
    ...
    return bookings
      ...
      .map((booking, idx) => {
        ...
        // Build rich event title: "Servicio · Cliente · Empleada"
        const clientName = booking.attendees?.[0]?.name;
        const staffName = booking.user?.name;
        const titleParts = [booking.title];
        if (clientName && !booking.title.includes(clientName)) {
          titleParts.push(clientName);
        }
        if (staffName) {
          titleParts.push(staffName);
        }
