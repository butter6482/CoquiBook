...
export default function Login({
  ...
  const formSchema = z
    .object({
      email: z.string().min(1, `${t("error_required_field")}`),
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    // Auto-append domain for username-only logins (e.g. "HBSTYLE" → "hbstyle@hb.pr")
    const emailValue = values.email.includes("@")
      ? values.email
      : `${values.email.toLowerCase()}@hb.pr`;
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center">
        ...
        <div className="w-full rounded-xl border border-subtle bg-default p-10 shadow-sm">
          ...
          <div className="mb-4 flex flex-col items-center gap-2">
            ...
            />
            <h1 className="font-cal text-lg font-bold text-emphasis">HB Style · CoquíBook</h1>
          </div>
